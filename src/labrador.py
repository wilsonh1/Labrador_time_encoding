...
class Labrador(nn.Module):
    def __init__(self, mask_token, pad_token, null_token, vocab_size, embedding_dim, transformer_heads, num_blocks, transformer_feedforward_dim, include_head, continuous_head_activation, dropout_rate):
        super().__init__()
        self.mask_token = mask_token
        self.pad_token = pad_token
        self.null_token = null_token
    ...
    def forward(self, categorical_input, continuous_input):
        ...
        x_continuous = self.continuous_embedding_layer(continuous_input)
